...
/**
 * An AttributeSource contains a list of different {@link AttributeImpl}s, and
 * methods to add and get them. There can only be a single instance of an
 * attribute in the same AttributeSource instance. This is ensured by passing in
 * the actual type of the Attribute (Class&lt;Attribute&gt;) to the
 * {@link #addAttribute(Class)}, which then checks if an instance of that type
 * is already present. If yes, it returns the instance, otherwise it creates a
...
public class AttributeSource
{
    ...
    public static abstract class AttributeFactory
    {
        ...
        /**
         * This is the default factory that creates {@link AttributeImpl}s using
         * the class name of the supplied {@link Attribute} interface class by
         * appending <code>Impl</code> to it.
         */
            ...
            private static Class<? extends AttributeImpl> getClassForInterface(Class<? extends Attribute> attClass)
            {
                synchronized (attClassImplMap)
                {
                    ...
                    if (clazz == null)
                    {
                        try
                        {
                            attClassImplMap.put(attClass, new WeakReference<Class<? extends AttributeImpl>>(clazz = Class.forName(attClass.getName() + "Impl", true, attClass.getClassLoader()).asSubclass(AttributeImpl.class)));
     ...
     * {@link AttributeSource.AttributeFactory#DEFAULT_ATTRIBUTE_FACTORY}.
     */
    ...
    /**
     * An AttributeSource using the supplied {@link AttributeFactory} for
     * creating new {@link Attribute} instances.
     */
    ...
    /**
     * Returns a new iterator that iterates the attribute classes in the same
    ...
    /**
     * Returns a new iterator that iterates all unique Attribute
     * implementations. This iterator may contain less entries that
     * {@link #getAttributeClassesIterator}, if one instance implements more
    ...
    static LinkedList<WeakReference<Class<? extends Attribute>>> getAttributeInterfaces(final Class<? extends AttributeImpl> clazz)
    {
        synchronized (knownImplClasses)
        {
            ...
            if (foundInterfaces == null)
            {
                ...
                // interfaces in the list (parameter "att"),
     ...
     * <p>
     * <font color="red"><b>Please note:</b> It is not guaranteed, that
     * <code>att</code> is added to the <code>AttributeSource</code>, because
     * the provided attributes may already exist. You should always retrieve the
     * wanted attributes using {@link #getAttribute} after adding with this
     * method and cast to your class. The recommended way to use custom
     * implementations is using an {@link AttributeFactory}. </font>
     * </p>
     */
    ...
    /**
     * The caller must pass in a Class&lt;? extends Attribute&gt; value. This
     * method first checks if an instance of that class is already in this
     * AttributeSource and returns it. Otherwise a new instance is created,
     * added to this AttributeSource and returned.
     */
    ...
    /**
     * The caller must pass in a Class&lt;? extends Attribute&gt; value. Returns
     * true, iff this AttributeSource contains the passed-in Attribute.
     */
    ...
    /**
     * The caller must pass in a Class&lt;? extends Attribute&gt; value. Returns
     * the instance of the passed in Attribute contained in this AttributeSource
     * 
     * @throws IllegalArgumentException
     *             if this AttributeSource does not contain the Attribute. It is
     *             recommended to always use {@link #addAttribute} even in
     *             consumers of TokenStreams, because you cannot know if a
     *             specific TokenStream really uses a specific Attribute.
     *             {@link #addAttribute} will automatically make the attribute
     *             available. If you want to only use the attribute, if it is
     *             available (to optimize consuming), use {@link #hasAttribute}.
     */
    ...
    /**
     * Restores this state by copying the values of all attribute
     * implementations that this state contains into the attributes
     * implementations of the targetStream. The targetStream must contain a
     * corresponding instance for each argument contained in this state (e.g. it
     * is not possible to restore the state of an AttributeSource containing a
     * TermAttribute into a AttributeSource using a Token instance as
     * implementation).
     * <p>
     * Note that this method does not affect attributes of the targetStream that
     * are not contained in this state. In other words, if for example the
     * targetStream contains an OffsetAttribute, but this state doesn't, then
     * the value of the OffsetAttribute remains unchanged. It might be desirable
     * to reset its value to the default, in which case the caller should first
     ...
     * 
     * <ul>
     * <li><em>iff {@code prependAttClass=true}:</em>
     * {@code "AttributeClass#key=value,AttributeClass#key=value"}
     * <li><em>iff {@code prependAttClass=false}:</em>
    ...
    /**
     * This method is for introspection of attributes, it should simply add the
     * key/values this AttributeSource holds to the given
     * {@link AttributeReflector}.
     * 
     * <p>
     ...
     * another TokenStream with exactly the same attributes (using
     * {@link #AttributeSource(AttributeSource)}). You can also use it as a
     * (non-performant) replacement for {@link #captureState}, if you need to
     * look into / modify the captured state.
     */
     ...
     * {@code AttributeSource}. The given instance has to provide all
     * {@link Attribute}s this instance contains. The actual attribute
     * implementations must be identical in both {@code AttributeSource}
     * instances; ideally both AttributeSource instances should use the same
     * {@link AttributeFactory}. You can use this method as a replacement for
     * {@link #restoreState}, if you use {@link #cloneAttributes} instead of
     * {@link #captureState}.
     */
