...
import static com.mogujie.jessica.util.ByteBlockPool.BYTE_BLOCK_MASK;
import static com.mogujie.jessica.util.ByteBlockPool.BYTE_BLOCK_SHIFT;
import static com.mogujie.jessica.util.ByteBlockPool.BYTE_BLOCK_SIZE;
 ...
 * efficiently in continuous storage. The mapping to the ordinal is encapsulated
 * inside {@link BytesRefHash} and is guaranteed to be increased for each added
 * {@link BytesRef}.
 * 
 * <p>
 * Note: The maximum capacity {@link BytesRef} instance passed to
 * {@link #add(BytesRef)} must not be longer than
 * {@link ByteBlockPool#BYTE_BLOCK_SIZE}-2. The internal storage is limited to
...
public final class BytesRefHash
{
    ...
    /**
     * Returns the number of {@link BytesRef} values in this
     * {@link BytesRefHash}.
     * 
     * @return the number of {@link BytesRef} values in this
     *         {@link BytesRefHash}.
     */
    public int size()
    {
     ...
     * 
     * @return the given BytesRef instance populated with the bytes for the
     *         given ord
     */
    ...
    /**
     * Returns the ords array in arbitrary order. Valid ords start at offset of
     * 0 and end at a limit of {@link #size()} - 1
     * <p>
     ...
     * @param bytes
     *            the bytes to hash
     * @return the ord the given bytes are hashed if there was no mapping for
     *         the given bytes, otherwise <code>(-(ord)-1)</code>. This
     *         guarantees that the return value will always be &gt;= 0 if the
     *         given bytes haven't been hashed before.
     * 
     ...
     * call. If {@link #clear()} has not been called previously this method has
     * no effect.
     */
    public void reinit()
    {
    ...
    public abstract static class BytesStartArray
    {
        ...
        /**
         * A {@link AtomicLong} reference holding the number of bytes used by
         * this {@link BytesStartArray}. The {@link BytesRefHash} uses this
         * reference to track it memory usage
         * 
         * @return a {@link AtomicLong} reference holding the number of bytes
         *         used by this {@link BytesStartArray}.
         */
        public abstract AtomicLong bytesUsed();
    }

    /**
     * A direct {@link BytesStartArray} that tracks all memory allocation using
     * an {@link AtomicLong} instance.
     */
    public static class TrackingDirectBytesStartArray extends BytesStartArray
    {
        ...
        public int[] init()
        {
            bytesStart = new int[ArrayUtil.oversize(initSize, RamUsageEstimator.NUM_BYTES_INT)];
            bytesUsed.addAndGet((bytesStart.length) * RamUsageEstimator.NUM_BYTES_INT);
            return bytesStart;
        }
